  ...
  return (
    <div className="space-y-4">
      ...
      <section>
        ...
        {scanComplete && <div className="mt-3 flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-xs leading-5 text-emerald-800"><Check className="mt-0.5 shrink-0" size={15} /><p><strong>Scan refill confirmed:</strong> Batch #MF-2026-X. Supply start updated in Clinician Triage Engine.</p><button aria-label="Dismiss confirmation" onClick={onDismiss} className="ml-auto text-emerald-500"><X size={14} /></button></div>}
      </section>
      <FollowUpCard />
      <TelemetryCard />
...
function HealthCard() {
  return (
    <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-slate-950 p-5 text-white shadow-lg shadow-indigo-900/20">
      ...
      <div className="relative mt-5 flex items-center gap-2 border-t border-white/15 pt-3 text-[10px] font-medium text-indigo-100"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_0_3px_rgba(110,231,183,0.15)]" /> DPDP Consent Active <span className="text-indigo-300">•</span> 3 Streams Connected</div>
    </section>
  );
}

function RegimenCard() {
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><div className="flex items-start justify-between"><div className="flex items-center gap-2"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><Pill size={17} /></span><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Active regimen</p><h2 className="mt-1 text-sm font-bold text-slate-900">Metformin Hydrochloride (500mg)</h2></div></div><span className="whitespace-nowrap rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-bold text-emerald-600">Active</span></div><div className="mt-3 flex items-center justify-between text-[10px]"><span className="font-medium text-slate-500">2x Daily</span><span className="flex items-center gap-1 text-slate-400"><Cloud size={12} /> Synced from WONDRx Smart Pen</span></div><div className="mt-4"><div className="mb-2 flex justify-between text-[10px] font-medium text-slate-500"><span>18 days remaining</span><span>60%</span></div><div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[60%] rounded-full bg-indigo-500" /></div><p className="mt-2 text-[10px] text-slate-400">Refill expected: <strong className="font-semibold text-slate-600">Oct 18</strong></p></div><div className="mt-4 flex items-start gap-2 border-t border-slate-100 pt-3 text-[10px] leading-4 text-slate-500"><CalendarDays size={14} className="mt-0.5 shrink-0 text-indigo-500" /> <span><strong className="font-semibold text-slate-700">Routine Follow-up: Oct 20</strong> (Aligned with 30-day supply window)</span></div></section>;
}

function FollowUpCard() {
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><CalendarDays size={16} /></span><h2 className="text-xs font-bold text-slate-800">Schedule & Doctor Follow-up</h2></div><div className="mt-3 rounded-xl bg-slate-50 px-3 py-3"><div className="flex justify-between"><div><p className="text-xs font-bold text-slate-800">Cardiology Follow-up</p><p className="mt-1 text-[10px] text-slate-400">Dr. Aisha Okonkwo · Cardiology</p></div><div className="text-right"><p className="text-xs font-bold text-indigo-600">Oct 20</p><p className="mt-1 font-mono text-[10px] text-slate-400">10:30 AM</p></div></div></div><p className="mt-3 flex items-center gap-2 text-[10px] font-semibold text-emerald-600"><TimerReset size={13} /> Cadence synced with active supply</p></section>;
}

function TelemetryCard() {
  return <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="rounded-lg bg-rose-50 p-2 text-rose-500"><HeartPulse size={17} /></span><h2 className="text-xs font-bold text-slate-800">Passive Telemetry</h2></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-bold text-emerald-600">Live</span></div><div className="mt-3 flex items-center justify-between rounded-xl bg-rose-50/70 px-3 py-3"><div><p className="flex items-center gap-1.5 text-[10px] font-medium text-slate-600"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Apple HealthKit Connected</p><p className="mt-1 text-xl font-bold tracking-tight text-slate-900">71 <span className="text-xs font-medium text-slate-500">bpm</span></p><p className="text-[10px] text-slate-500">Resting Heart Rate: Normal Baseline</p></div><Activity size={58} strokeWidth={1.5} className="text-rose-400" /></div><p className="mt-3 text-[10px] text-slate-400">Passive adherence inference active. No manual checklists required.</p></section>;
}
...
function ConsentCard({ open, setOpen, consents, toggleConsent }: { open: boolean; setOpen: (value: boolean) => void; consents: { biometrics: boolean; rx: boolean; history: boolean }; toggleConsent: (key: 'biometrics' | 'rx' | 'history') => void }) {
  return <section className="rounded-2xl border border-slate-100 bg-white shadow-sm"><button onClick={() => setOpen(!open)} className="flex w-full items-center justify-between px-4 py-4 text-left"><span className="flex items-center gap-2"><span className="rounded-lg bg-slate-100 p-2 text-slate-600"><KeyRound size={16} /></span><span className="text-xs font-bold text-slate-800">Data Privacy & Consent</span></span><ChevronRight size={16} className={`text-slate-400 transition ${open ? 'rotate-90' : ''}`} /></button>{open && <div className="px-4 pb-3"> <ConsentRow label="Share Continuous Biometrics with Doctor" detail="Heart rate and activity" checked={consents.biometrics} onChange={() => toggleConsent('biometrics')} /><ConsentRow label="Sync Chemist 'Rx Served' Events" detail="WONDRx prescription data" checked={consents.rx} onChange={() => toggleConsent('rx')} /><ConsentRow label="Share Cross-Clinic History" detail="Care coordination across clinics" checked={consents.history} onChange={() => toggleConsent('history')} /></div>}</section>;
}
...
function SettingsSubpageView({ page, onBack }: { page: SettingsSubpage; onBack: () => void }) {
  const content: Record<SettingsSubpage, { title: string; subtitle: string; heading: string; detail: string }> = {
    consent: { title: 'Consent connections', subtitle: 'Granular control over shared health data', heading: 'Your active permissions', detail: 'Review and adjust the three consent streams connected to DoseTrace. Changes apply immediately to future health data syncs.' },
    services: { title: 'Connected services', subtitle: 'Apps and devices linked to DoseTrace', heading: '3 streams connected', detail: 'WONDRx Smart Pen, Apple HealthKit, and your outpatient clinic are currently connected for coordinated care.' },
    security: { title: 'Security', subtitle: 'How your health data stays protected', heading: 'End-to-end encrypted', detail: 'Your Universal Health ID and connected health streams use encrypted transfer and anonymized patient identifiers.' },
    clinic: { title: 'Outpatient Cardiology Unit', subtitle: 'Active clinic', heading: 'Your care team', detail: 'This clinic can access the care data you have explicitly shared, including active prescriptions, telemetry, and relevant lab reports.' },
    doctor: { title: 'Dr. Aisha Okonkwo', subtitle: 'Cardiology · Outpatient Cardiology Unit', heading: 'Next appointment · Oct 20', detail: 'Your routine follow-up is scheduled for 10:30 AM. Your active supply window and latest telemetry will be ready for review.' },
  };
